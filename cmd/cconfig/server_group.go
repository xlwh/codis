...
func cmdServer(argv []string) (err error) {
	...
	codis-config server add <group_id> <redis_addr> <role> <tag>
	codis-config server remove <group_id> <redis_addr>
	codis-config server promote <group_id> <redis_addr> <mode>
	codis-config server add-group <group_id>
	codis-config server remove-group <group_id>
	codis-config server failover <group_id> <mode> <tag>
	codis-config server mode <group_id> <redis_addr> <mode>
`
	...
	if args["failover"].(bool) {
		return runFailover(groupId, args["<mode>"].(string), args["<tag>"].(string))
	}
	...
	if args["add"].(bool) {
		...
		tag := args["<tag>"].(string)
		return runAddServerToGroup(groupId, serverAddr, role, tag)
	}
	if args["remove"].(bool) {
		return runRemoveServerFromGroup(groupId, serverAddr)
	}
	if args["promote"].(bool) {
		return runPromoteServerToMaster(groupId, serverAddr, args["<mode>"].(string))
	}
	if args["mode"].(bool) {
		return runChangeServerMode(groupId, serverAddr, args["<mode>"].(string))
	}
...
func runPromoteServerToMaster(groupId int, addr string, mode string) error {
	s := models.Server{
		Addr:    addr,
		GroupId: groupId,
	}
	var v interface{}
	err := callApi(METHOD_POST, fmt.Sprintf("/api/server_group/%d/promote/%s", groupId, mode), s, &v)
	if err != nil {
		return err
	}
	fmt.Println(jsonify(v))
...
func runFailover(groupId int, mode string, tag string) error {
	var v interface{}
	err := callApi(METHOD_POST, fmt.Sprintf("/api/server_group/%d/failover/%s/%s", groupId, mode, tag), nil, &v)
	if err != nil {
		return err
	}
	fmt.Println(jsonify(v))
...
func runChangeServerMode(groupId int, addr string, mode string) error {
	server := models.NewServer(mode, addr, "")
	var v interface{}
	err := callApi(METHOD_PUT, fmt.Sprintf("/api/server_group/%d/serverModeCtrl", groupId), server, &v)
	if err != nil {
		return err
	}
	fmt.Println(jsonify(v))
...
func runAddServerToGroup(groupId int, addr string, role string, tag string) error {
	server := models.NewServer(role, addr, tag)
	var v interface{}
