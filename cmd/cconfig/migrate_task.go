...
func (task *MigrateTask) Migrate(slot *models.Slot, fromGroup, toGroup int, onProgress func(SlotMigrateProgress)) (err error) {
	...
	/* set ssdb status */
	/* first, flag improting , then flag migrating */
	/* do not check err for the sake of reentrance */
	/* do not reset flag when come up to an error as a result of consistence */
	log.Infof("migrate %d from %s to %s", slot.Id, fromMaster.Addr, toMaster.Addr)
	if err := utils.SlotPreImporting(toMaster.Addr, globalEnv.Password(), slot.Id, toMaster.Addr); err != nil {
		log.Warnf("set slot %d pre-importing at %s failed - %s", slot.Id, toMaster.Addr, err)
	}
	if err := utils.SlotPreMigrating(fromMaster.Addr, globalEnv.Password(), slot.Id, fromMaster.Addr); err != nil {
		log.Warnf("set slot %d pre-migrating at %s failed - %s", slot.Id, fromMaster.Addr, err)
	}
	//

	_, remain, err := utils.SlotsMgrtTagSlot(fromMaster.Addr, globalEnv.Password(), slot.Id, toMaster.Addr)
	if err != nil {
		return err
	}
	...
	/* unset ssdb status */
	if err := utils.SlotPostMigrating(fromMaster.Addr, globalEnv.Password(), slot.Id, fromMaster.Addr); err != nil {
		log.Warnf("set slot %d post-migrating at %s failed - %s", slot.Id, fromMaster.Addr, err)
	}
	if err := utils.SlotPostImporting(toMaster.Addr, globalEnv.Password(), slot.Id, toMaster.Addr); err != nil {
		log.Warnf("set slot %d post-importing at %s failed - %s", slot.Id, toMaster.Addr, err)
	}
	return nil
}
