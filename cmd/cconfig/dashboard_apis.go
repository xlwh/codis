...
func apiEnv() (int, string) {
	m := globalEnv.(*CodisEnv).WriteMap()
	b, err := json.MarshalIndent(m, " ", "  ")
	if err != nil {
		return 500, err.Error()
	}
...
type GroupList []*models.ServerGroup
...
func apiInitSlots(r *http.Request) (int, string) {
	...
	/* if epic is not 0(cluster is running), disable this command */
	epic, err := models.GetEpic(safeZkConn, globalEnv.ProductName())
	if err != nil {
		log.ErrorErrorf(err, "check epic failed")
		return 500, err.Error()
	}
	log.Infof("epic %v", epic)
	if epic != 0 {
		return 500, "can't init slots while cluster is running"
	}
...
func apiCleanSlots(r *http.Request) (int, string) {
	/*r.ParseForm()
	isForce := false
	val := r.FormValue("is_force")
	if len(val) > 0 && (val == "1" || val == "true") {
	...
	if !isForce {
		info, err := models.ProxyList(safeZkConn, globalEnv.ProductName(), nil)
		if err != nil {
			log.ErrorErrorf(err, "list proxy failed")
			return 500, err.Error()
		}
		if len(info) > 0 {
			return 500, "slots already initialized, you may use 'is_force' flag and try again."
	...
	if err := models.CleanSlotSet(safeZkConn, globalEnv.ProductName()); err != nil {
		log.ErrorErrorf(err, "clean slot set failed")
		return 500, err.Error()
	}
	return jsonRetSucc()*/
	return 500, "deprecated"
}
...
func apiPromoteServer(server models.Server, param martini.Params) (int, string) {
	...
	psync := (strings.ToUpper(param["mode"]) == "PSYNC")

	if err := group.Promote(safeZkConn, server.Addr, globalEnv.Password(), psync); err != nil {
		log.ErrorErrorf(err, "promote group %d failed, psync: %v", server.GroupId, psync)
		return 500, err.Error()
	}
...
func apiFailover(param martini.Params) (int, string) {
	id, _ := strconv.Atoi(param["id"])
	tag := param["tag"]
	mode := param["mode"]
	lock := utils.GetZkLock(safeZkConn, globalEnv.ProductName())
	if err := lock.LockWithTimeout(0, fmt.Sprintf("failover id: %d mode: %s tag: %s", id, mode, tag)); err != nil {
		return 500, err.Error()
	}
	defer func() {
		err := lock.Unlock()
	...
	group, err := models.GetGroup(safeZkConn, globalEnv.ProductName(), id)
	if err != nil {
		log.ErrorErrorf(err, "get group %d failed", id)
		return 500, err.Error()
	}

	psync := (strings.ToUpper(mode) == "PSYNC")

	if addr, err := group.Failover(safeZkConn, globalEnv.Password(), psync, tag); err != nil {
		log.ErrorErrorf(err, "failover id: %d mode: %s tag: %s failed", id, mode, tag)
		return 500, err.Error()
	} else {
		log.Infof("failover id: %d to %s mode: %s tag; %s done", id, addr, mode, tag)
...
func apiServerModeCtrl(server models.Server, param martini.Params) (int, string) {
	groupid, _ := strconv.Atoi(param["id"])
	mode := server.Type
	lock := utils.GetZkLock(safeZkConn, globalEnv.ProductName())
	if err := lock.LockWithTimeout(0, fmt.Sprintf("disable server from group , %+v", server)); err != nil {
		return 500, err.Error()
	}
	defer func() {
		err := lock.Unlock()
	...
	serverGroup := models.NewServerGroup(globalEnv.ProductName(), groupid)
	if strings.ToUpper(mode) == "DISABLE" {
		if err := serverGroup.DisableServer(safeZkConn, server.Addr); err != nil {
			log.ErrorErrorf(err, "disable server %s from group %d failed", server.Addr, groupid)
			return 500, err.Error()
		}
	} else {
		if err := serverGroup.EnableServer(safeZkConn, server.Addr); err != nil {
			log.ErrorErrorf(err, "enable server %s from group %d failed", server.Addr, groupid)
			return 500, err.Error()
		}
	}
	return jsonRetSucc()
...
func apiRemoveServerFromGroup(server models.Server, param martini.Params) (int, string) {
	...
	if err != nil {
		log.ErrorErrorf(err, "remove server %s from group %d failed", server.Addr, groupId)
		return 500, err.Error()
	}
	return jsonRetSucc()
}
...
func apiGetSlotsEx() (int, string) {
	addrs := globalEnv.CollectorAddr()
	if len(addrs) > 0 {
		m, err := getMeta(globalEnv.ProductName(), strings.Split(addrs, ","))
		if err != nil {
			return 500, err.Error()
		}
		...
		b, err := json.MarshalIndent(slots, " ", "  ")
		if err != nil {
			log.ErrorErrorf(err, "Encode slots info failed")
			return 500, err.Error()
		}
		return 200, string(b)
	} else {
