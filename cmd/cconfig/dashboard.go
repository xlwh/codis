...
func createDashboardNode() error {
	...
	for {
		if exists, _, err := safeZkConn.Exists(zkPath); err == nil && exists {
			log.Info("Check dashboard status")
		} else if err == nil {
			log.Infof("I am online")
			break
		} else {
	...
	pathCreated, err := safeZkConn.Create(zkPath, []byte(content), FlagEphemeral, zkhelper.DefaultFileACLs())
...
func runDashboard(addr string, httpLogFile string) {
	...
	m.Post("/api/server_group/(?P<id>[0-9]+)/promote/:mode", binding.Json(models.Server{}), apiPromoteServer)
	m.Post("/api/server_group/(?P<id>[0-9]+)/failover/:mode/:tag", apiFailover)
	m.Put("/api/server_group/(?P<id>[0-9]+)/serverModeCtrl", binding.Json(models.Server{}), apiServerModeCtrl)
	...
	m.Get("/api/slot/list", apiGetSlotsEx)
	m.Get("/api/slot/:id", apiGetSingleSlot)
	m.Post("/api/slots/init", apiInitSlots)
	m.Post("/api/slots/clean", apiCleanSlots)
	m.Get("/api/slots", apiGetSlotsEx)
	...
	*/
	log.Infof("Start HA Monitor routine")
	go RunHaMonitor()
