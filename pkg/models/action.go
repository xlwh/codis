...
func InitEpic(zkConn zkhelper.Conn, product string) error {
	path := GetEpicPath(product)
	epic := &Epic{}
	epic.V = 1

	b, err := json.Marshal(epic)
	if err != nil {
		return errors.Trace(err)
	}

	if _, err := zkConn.Set(path, b, -1); err != nil {
		return errors.Trace(err)
	}
...
func IncEpic(zkConn zkhelper.Conn, product string) error {
	path := GetEpicPath(product)
	b, _, err := zkConn.Get(path)
	if err != nil {
		return errors.Trace(err)
	}
	...
	if _, err := zkhelper.CreateOrUpdate(zkConn, path, string(b), 0, zkhelper.DefaultDirACLs(), true); err != nil {
		return errors.Trace(err)
	}
