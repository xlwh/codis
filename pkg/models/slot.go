...
func SetSlotRange(zkConn zkhelper.Conn, productName string, fromSlot, toSlot, groupId int, status SlotStatus) error {
	...
	for i := fromSlot; i <= toSlot; i++ {
		...
		group, err := GetGroup(zkConn, productName, groupId)
		if err != nil {
			return errors.Trace(err)
		}

		master, err := group.Master(zkConn)
		if err != nil {
			return errors.Trace(err)
		}

		if err := utils.SetSlot(master.Addr, i); err != nil {
			return errors.Trace(err)
		}
	}

	/* increase epci to trigger collector update */
	err = IncEpic(zkConn, productName)
...
func InitSlotSet(zkConn zkhelper.Conn, productName string, totalSlotNum int) error {
	if err := InitEpic(zkConn, productName); err != nil {
		return errors.Trace(err)
	}
...
func CleanSlotSet(zkConn zkhelper.Conn, productName string) error {
	zkPath := GetSlotBasePath(productName)
	children, _, err := zkConn.Children(zkPath)
	if err != nil {
		return errors.Trace(err)
	...
	for _, p := range children {
		if err := zkConn.Delete(path.Join(zkPath, p), -1); err != nil {
			return errors.Trace(err)
		}
	}
	return nil
...
func (s *Slot) Update(zkConn zkhelper.Conn) error {
	...
	/* increase epci to trigger collector update */
	err = IncEpic(zkConn, s.ProductName)
	if err != nil {
		return errors.Trace(err)
	}
	return nil
