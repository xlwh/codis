...
import (
	...
	"github.com/ksarch-saas/codis/pkg/utils/trace"
)
...
func Init(path, prefix string, flags int, level, trace LogLevel) {
	StdLog.path = path
	StdLog.level = level
	StdLog.trace = trace
	StdLog.open()
...
func (l *Logger) open() {
	...
	if err != nil {
		l.F = log.New(os.Stdout, "", LstdFlags)
		l.f = nil
	}
	l.F = log.New(f, "", LstdFlags)
	l.f = f

	wf, err := os.OpenFile(l.path+".wf", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0644)
	if err != nil {
		l.WF = log.New(os.Stderr, "", LstdFlags)
		l.wf = nil
	}
	l.WF = log.New(wf, "", LstdFlags)
	l.wf = wf

	sf, err := os.OpenFile(l.path+".slow", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0644)
	if err != nil {
		l.SF = log.New(os.Stderr, "", LstdFlags)
		l.sf = nil
	}
	l.SF = log.New(sf, "", LstdFlags)
	l.sf = sf
}
...
func (l *Logger) SetPrefix(prefix string) {
	l.F.SetPrefix(prefix)
	l.WF.SetPrefix(prefix)
	l.SF.SetPrefix(prefix)
}
...
func (l *Logger) output(traceskip int, err error, t LogType, s string) error {
	...
	/* check reopen */
	if atomic.CompareAndSwapInt32(&l.reopen, 1, 0) {
		l.close()
		l.open()
	}

	if t == TYPE_SLOW {
		return l.SF.Output(0, s)
	}

	if t > TYPE_WARN {
		return l.F.Output(traceskip+2, s)
	}
	return l.WF.Output(traceskip+2, s)
}

func Flags() int {
	return StdLog.F.Flags()
}

func Prefix() string {
	return StdLog.F.Prefix()
}

func SetFlags(flags int) {
	StdLog.F.SetFlags(flags)
	StdLog.WF.SetFlags(flags)
	StdLog.SF.SetFlags(flags)
}

func SetPrefix(prefix string) {
	StdLog.F.SetPrefix(prefix)
	StdLog.WF.SetPrefix(prefix)
	StdLog.SF.SetPrefix(prefix)
}
